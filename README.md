...
[![Tested with Pytest](https://img.shields.io/badge/testing-Pytest-00c0ff.svg?style=for-the-badge)](https://github.com/AtlasRoX/Helix/actions/workflows/tests.yml)
...
<div align="center">
  <a href="https://star-history.com/#AtlasRoX/Helix&Date">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=AtlasRoX/Helix&type=Date&theme=dark">
      <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=AtlasRoX/Helix&type=Date">
      <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=AtlasRoX/Helix&type=Date" width="700">
    </picture>
  </a>
</div>
...
```bash
curl -fsSL "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.sh?raw=1" | sh
```

Windows PowerShell:

```powershell
irm "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.ps1?raw=1" | iex
```

Review the installers at [scripts/install.sh](https://github.com/AtlasRoX/Helix/blob/main/scripts/install.sh) and [scripts/install.ps1](https://github.com/AtlasRoX/Helix/blob/main/scripts/install.ps1).
...
`gc-claud` reads the current configured port and auth token each time it starts, sets the Claude Code environment variables (including a 190k-token `CLAUDE_CODE_AUTO_COMPACT_WINDOW` for auto-compaction), and then launches the real `claude` command.
...
Keep `gc-server` running while you work. The Admin UI manages proxy config, restarts the server when runtime settings change, and `gc-claud` reads the current Admin UI-managed port and auth token every time it starts. It also sets `CLAUDE_CODE_AUTO_COMPACT_WINDOW` to `190000` for auto-compaction.
...
1. With `gc-server` running, open the **Admin UI** URL from the terminal output.
...
curl -fsSL "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.sh?raw=1" | sh -s -- --voice-nim

# Local Whisper (CPU or CUDA)
curl -fsSL "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.sh?raw=1" | sh -s -- --voice-local

# Both backends
curl -fsSL "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.sh?raw=1" | sh -s -- --voice-all

# Local Whisper with CUDA
curl -fsSL "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.sh?raw=1" | sh -s -- --voice-local --torch-backend cu130
```
...
```powershell
# NVIDIA NIM transcription (Riva gRPC)
& ([scriptblock]::Create((irm "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.ps1?raw=1"))) -VoiceNim

# Local Whisper (CPU or CUDA)
& ([scriptblock]::Create((irm "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.ps1?raw=1"))) -VoiceLocal

# Both backends
& ([scriptblock]::Create((irm "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.ps1?raw=1"))) -VoiceAll

# Local Whisper with CUDA
& ([scriptblock]::Create((irm "https://github.com/AtlasRoX/Helix/blob/main/scripts/install.ps1?raw=1"))) -VoiceLocal -TorchBackend cu130
```

Restart `gc-server` after reinstalling.
...
```bash
git clone https://github.com/AtlasRoX/Helix.git
cd Helix
...
- `gc-server`: starts the proxy with configured host and port.
- `gc-claud`: launches Claude Code with the configured local proxy URL, auth token, model discovery flag, and a 190k `CLAUDE_CODE_AUTO_COMPACT_WINDOW` for auto-compaction.
- `fcc-server`: compatibility alias for `gc-server`.
- `fcc-claude`: compatibility alias for `gc-claud`.
- `free-claude-code`: compatibility alias for `gc-server`.
...
- Report bugs and feature requests in [Issues](https://github.com/AtlasRoX/Helix/issues).
